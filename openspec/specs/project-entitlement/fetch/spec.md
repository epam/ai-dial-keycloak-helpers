# project-entitlement/fetch Specification

## Purpose
Caches the brokered user's project entitlement: at every brokered login the
fetch mapper calls Microsoft Graph with the user's own delegated token and
stores the convention-gated group values (plus a fetch timestamp) on the
Keycloak user, failing closed on every unverifiable membership.
